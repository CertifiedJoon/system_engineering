/*
File Name       : llama2_3035666657.c
UID             : 3035666657
Dev Platform    : VScode
Remark          : Documentation, Report, Implementation 3 Done.
*/
...
struct rusage* child_usages; // get usage for each thread
...
typedef struct __mult_vec_param {
    ...
    int rows_per_thread;
    float* val;
} mult_vec_param; // struct of arguments passed to threads

pthread_t* threads; // stores threads
int n = 0; // total number of threads
mult_vec_param* params; // store arguments passed to threads

sem_t* child; // signal child to start
sem_t parent; // signal main that child is done
sem_t sem_param; // lock params
sem_t sem_term; // lock terminated constant

int terminated = 0; // generation terminated flag
...
void *thr_func(void *arg) {
    ...
    while (terminated == 0) {
        // wait untill main thread signals to start
        ...
        // signal main thread that multiplication is done
        sem_post(&parent);
    }

    // get rusage for this thread
...
int init_mat_vec_mul(int thr_count) {
    ...
    for (int i = 0; i < thr_count; i++) {
        // workaround to pass deep copy of i
...
void mat_vec_mul(float* out, float* vec, float* mat, int col, int row) {
    ...
    // calculate how many rows should a thread handle
    if (row % n == 0) {
